    ...
    try:
        ...
        if TOKEN:
            ...
            if utils2.menu_on('liked_lists'):
                add_refresh_item({'mode': MODES.LIKED_LISTS, 'section': section}, i18n('liked_lists'), utils2.art('liked_lists.png'), fanart=utils2.art('fanart.jpg'))
        ...
        logger.log('Error in browse_menu: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
        kodi.end_of_directory()

@url_dispatcher.register(MODES.GENRES, ['section'])
def browse_genres(section):
    genres = trakt_api.get_genres(section)
    for genre in genres:
        # Debugging: Log the type and content of genre
        logger.log('Genre type: %s, Genre content: %s' % (type(genre), genre), log_utils.LOGDEBUG)
        
        if genre == 'none':
            continue
        kodi.create_item({'mode': MODES.SHOW_GENRE, 'genre': genre, 'section': section}, genre.capitalize(), utils2.art('%s.png' % (genre)), fanart=utils2.art('fanart.jpg'))
    kodi.set_content(CONTENT_TYPES.ADDONS)
    kodi.end_of_directory()
...
def add_section_lists(section):
    try:
        ...
        main_list = main_str.split('|') if main_str else []
        other_dict = dict(('%s@%s' % (item[1], item[0]), item) for item in db_connection.get_other_lists(section))
        lists_dict = dict((user_list['ids']['slug'], user_list) for user_list in trakt_api.get_lists()) if TOKEN else {}
        ...
        logger.log('Error in add_section_lists: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
            #add_other_list_item(MODES.BROWSE, section, other_list)
...
def add_refresh_item(queries, label, thumb, fanart):
    try:
        ...
        if 'section' in queries:
            refresh_queries.update({'section': queries['section']})
        ...
        logger.log('Error in add_refresh_item: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def add_search_item(queries, label, thumb, clear_mode):
    try:
        ...
        logger.log('Error in add_search_item: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def browse_trending(section, page=1):
    try:
        ...
        logger.log('Error in browse_trending: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def browse_popular(section, page=1):
    try:
        ...
        logger.log('Error in browse_popular: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def browse_anticipated(section, page=1):
    try:
        ...
        logger.log('Error in browse_anticipated: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def browse_recent(section, page=1):
    try:
        ...
        logger.log('Error in browse_recent: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def browse_recommendations(section):
    try:
        ...
        logger.log('Error in browse_recommendations: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def show_history(section, page=1):
    try:
        ...
        logger.log('Error in show_history: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
        kodi.end_of_directory()
...
def toggle_to_menu(action, section, slug, username=None):
    try:
        list_str = slug if username is None else '%s@%s' % (slug, username)
        ...
        if action == 'add':
            ...
            main_list = [item for item in main_list if item != list_str]

        kodi.set_setting(setting, '|'.join(main_list))
        kodi.refresh_container()
    except Exception as e:
        logger.log('Error in toggle_to_menu: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def search(section, search_text=None):
    try:
        ...
        logger.log('Error in search: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
...
def saved_searches(section):
    try:
        ...
        logger.log('Error in saved_searches: %s' % e, log_utils.LOGERROR)
        kodi.notify(msg='[COLOR red]Error:[/COLOR] %s' % e, duration=5000)
        kodi.end_of_directory()
