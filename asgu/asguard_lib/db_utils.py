...
class DB_Connection():
    ...
    def get_cached_url(self, url, data='', cache_limit=8):
        if data is None:
            data = ''
        # Truncate data if running MySQL and greater than column size
        if self.db_type == DB_TYPES.MYSQL and len(data) > MYSQL_DATA_SIZE:
            data = data[:MYSQL_DATA_SIZE]
    ...
    def cache_function(self, name, args=None, kwargs=None, result=None):
        ...
        arg_hash = hashlib.md5(str(args).encode('utf-8')).hexdigest() + hashlib.md5(str(kwargs).encode('utf-8')).hexdigest()
...
from sqlite3 import dbapi2 as db_lib  # @Reimport
def import_cache_to_db(cache_db_path, main_db_path):
    try:
        # Connect to the cache database
        cache_conn = db_lib.connect(cache_db_path)
        cache_cursor = cache_conn.cursor()

        # Connect to the main database
        main_conn = db_lib.connect(main_db_path)
        ...
        # Fetch all data from the cache database
        cache_cursor.execute("SELECT key, value, date FROM cache")
        ...
        logger.log('Successfully imported cache data to main database', log_utils.LOGDEBUG)
    except Exception as e:
        logger.log('Error importing cache data to main database: %s' % e, log_utils.LOGERROR)