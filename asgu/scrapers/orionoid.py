...
class Scraper(scraper.Scraper):
	...
	def _error(self):
		...
		xbmc.log('ASGUARD ORION [ERROR]: ' + parameters, xbmc.LOGERROR)
	...
	def _premiumizeParameters(self, parameters = None):
		...
		if parameters: parameters = [urllib_parse.urlencode(parameters, doseq = True)]
		else: parameters = []
		parameters.append(urllib_parse.urlencode({'access_token' : PremiumizeMeResolver.get_setting('token')}, doseq = True))
